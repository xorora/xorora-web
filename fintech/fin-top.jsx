// Xorora — Fintech Industry Page · header with glass form
const { useState: useFinTop } = React;

function FinHeader({ onCTA }) {
  return (
    <section style={{ position: 'relative', overflow: 'hidden', background: 'var(--navy-900)' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: "url('assets/fintech/header-bg.jpg')", backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.5, pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(95deg, rgba(1,12,40,0.97) 0%, rgba(1,12,40,0.9) 42%, rgba(3,12,38,0.66) 72%, rgba(3,12,38,0.52) 100%)', pointerEvents: 'none' }} />
      ...
      <div style={{ position: 'relative', maxWidth: 1240, margin: '0 auto', padding: 'clamp(120px,12vw,150px) 32px clamp(72px,8vw,104px)', display: 'grid', gridTemplateColumns: '1.08fr 0.92fr', gap: 'clamp(40px,5vw,72px)', alignItems: 'center' }} className="fin-header-grid">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 26, fontFamily: 'Poppins, sans-serif', fontSize: 13.5, color: 'rgba(255,255,255,.6)' }}>
            ...
            <Chevron size={13} color="rgba(255,255,255,.4)" />
            <span style={{ color: '#fff', fontWeight: 600 }}>Fintech</span>
          </div>
          ...
          <h1 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(36px,4.8vw,60px)', lineHeight: 1.02, letterSpacing: '-0.025em', color: '#fff', margin: '0 0 26px', textWrap: 'balance' }}>
            Custom fintech software for future driven companies<span style={{ color: 'var(--tangerine-400)' }}>.</span>
          </h1>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 22, flexWrap: 'wrap' }}>
            {[['shield-check', 'PCI DSS ready'], ['lock', 'PSD2 & GDPR'], ['award', 'ISO 27001']].map(([ic, l]) => (
...
function FinGlassForm() {
  const [sent, setSent] = useFinTop(false);
  const [nda, setNda] = useFinTop(false);
  return (
    <div id="fin-form" style={{
        ...
        {sent ? (
          ...
          <React.Fragment>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
              <FinField label="Full Name" placeholder="Jordan Reyes" />
              <FinField label="Email" placeholder="you@company.com" />
            </div>
            <div style={{ marginTop: 18 }}><FinField label="About project" placeholder="Tell us what you want to build" textarea /></div>
...
function FinField({ label, placeholder, textarea }) {
  const [f, setF] = useFinTop(false);
  const common = {
...
Object.assign(window, { FinHeader });
