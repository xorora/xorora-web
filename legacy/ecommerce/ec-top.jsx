// Xorora — E-commerce Industry Page · header with glass form
const { useState: useEcTop } = React;

function EcHeader({ onCTA }) {
  return (
    <section style={{ position: 'relative', overflow: 'hidden', background: 'var(--navy-900)' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: "url('assets/ecommerce/header-bg.jpg')", backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.5, pointerEvents: 'none' }} />
      ...
      <div style={{ position: 'relative', maxWidth: 1240, margin: '0 auto', padding: 'clamp(120px,12vw,150px) 32px clamp(72px,8vw,104px)', display: 'grid', gridTemplateColumns: '1.08fr 0.92fr', gap: 'clamp(40px,5vw,72px)', alignItems: 'center' }} className="ec-header-grid">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 26, fontFamily: 'Poppins, sans-serif', fontSize: 13.5, color: 'rgba(255,255,255,.6)' }}>
            ...
            <Chevron size={13} color="rgba(255,255,255,.4)" />
            <span style={{ color: '#fff', fontWeight: 600 }}>E-commerce</span>
          </div>
          ...
          <h1 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(36px,4.8vw,60px)', lineHeight: 1.02, letterSpacing: '-0.025em', color: '#fff', margin: '0 0 26px', textWrap: 'balance' }}>
            Custom e-commerce software for growth driven brands<span style={{ color: 'var(--tangerine-400)' }}>.</span>
          </h1>
...
function EcGlassForm() {
  const [sent, setSent] = useEcTop(false);
  const [nda, setNda] = useEcTop(false);
  return (
    <div id="ec-form" style={{
        ...
        {sent ? (
          ...
          <React.Fragment>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
              <EcField label="Full Name" placeholder="Jordan Reyes" />
              <EcField label="Email" placeholder="you@company.com" />
            </div>
            <div style={{ marginTop: 18 }}><EcField label="About project" placeholder="Tell us what you want to build" textarea /></div>
...
function EcField({ label, placeholder, textarea }) {
  const [f, setF] = useEcTop(false);
  const common = {
