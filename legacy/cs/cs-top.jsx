...
function Header({ onCTA }) {
  return (
    <section style={{ position: 'relative', overflow: 'hidden', background: 'var(--navy-900)' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: "url('assets/header-bg.jpg')", backgroundSize: 'cover', backgroundPosition: 'center right', opacity: 0.6, pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(1,12,40,0.96) 0%, rgba(1,12,40,0.82) 36%, rgba(1,12,40,0.30) 66%, rgba(1,12,40,0.55) 100%)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(1,12,40,0.45) 0%, rgba(1,12,40,0) 26%, rgba(2,6,15,0.85) 100%)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(78% 70% at 80% 2%, rgba(91,141,239,0.22), transparent 60%)', pointerEvents: 'none' }} />
      <div style={{ position: 'relative', maxWidth: 1240, margin: '0 auto', padding: 'clamp(120px,12vw,150px) 32px clamp(68px,8vw,108px)', display: 'grid', gridTemplateColumns: '1.04fr 0.96fr', gap: 'clamp(40px,5vw,72px)', alignItems: 'center' }} className="cs-header-grid">
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, padding: '8px 8px 8px 16px', borderRadius: 'var(--r-pill)', border: '1px solid rgba(255,255,255,.18)', background: 'rgba(255,255,255,.06)', backdropFilter: 'blur(6px)', marginBottom: 30 }}>
            <span style={{ fontFamily: 'Poppins, sans-serif', fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,.9)' }}>AI Voice Automation</span>
          ...
          <h1 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(36px,5vw,62px)', lineHeight: 1.0, letterSpacing: '-0.025em', color: '#fff', margin: '0 0 24px', textWrap: 'balance' }}>
            From fragmented tools to unified <span style={{ background: 'linear-gradient(100deg, #C3C6EC, #6F74C9 52%, #FF8A3D)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>AI voice operations<span style={{ color: 'var(--tangerine-400)', WebkitTextFillColor: 'var(--tangerine-400)' }}>.</span></span>
          </h1>
          <p style={{ fontFamily: 'Poppins, sans-serif', fontSize: 18.5, lineHeight: 1.62, color: 'rgba(255,255,255,.74)', maxWidth: 540, margin: '0 0 36px' }}>
          ...
          <div style={{ display: 'flex', gap: 40, marginBottom: 38, flexWrap: 'wrap' }}>
            {[['4', 'Role-specific portals'], ['16mo', 'Concept to production']].map(([v, l]) => (
              <div key={l}>
                <div style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 42, letterSpacing: '-0.03em', color: '#fff', lineHeight: 1 }}>{v}</div>
                <div style={{ fontFamily: 'Poppins, sans-serif', fontSize: 14, color: 'rgba(255,255,255,.55)', marginTop: 8 }}>{l}</div>
              </div>
            ))}
          </div>
        ...
        <div style={{ position: 'relative' }} className="cs-header-art">
          <BrowserFrame src="assets/mocks/portal-business.webp" alt="Business portal dashboard" url="app.xorora.ai/dashboard" glow />
