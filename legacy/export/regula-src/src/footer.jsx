...
function Footer({ onNav }) {
  const cols = [
    { h: 'AI & Data Innovation', items: ['Applied ML & LLMs', 'RAG & agents', 'Computer vision', 'Predictive analytics', 'Model evaluation'] },
    { h: 'Engineering', items: ['AI product engineering', 'Platform & APIs', 'MLOps & monitoring', 'Embedded teams', 'Modernization'] },
    { h: 'Cloud & Security', items: ['Cloud architecture', 'Data foundations', 'Guardrails & governance', 'SOC 2 & compliance', 'Cost optimization'] },
    { h: 'Company', items: ['About Xorora', 'How we build', 'Careers', 'Case studies', 'Contact us'] },
  ];
  const offices = [
  ...
  return (
    <footer style={{ background: 'var(--navy-950)', borderTop: '1px solid rgba(255,255,255,.07)' }}>
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: '72px 32px 48px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 40 }} className="xo-footer-grid">
          {cols.map(c => (
            <div key={c.h}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {c.items.map(i => (
                  <a key={i} onClick={() => onNav('top')} style={{ fontFamily: 'Poppins, sans-serif', fontSize: 14, color: 'rgba(255,255,255,.6)', textDecoration: 'none', cursor: 'pointer', transition: 'color .15s', width: 'fit-content' }}
                    onMouseEnter={e => e.currentTarget.style.color = 'var(--tangerine-400)'}
                    onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,255,255,.6)'}>{i}</a>
                ))}
              </div>
            </div>
          ))}
