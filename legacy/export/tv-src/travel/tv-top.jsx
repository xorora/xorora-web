// Xorora — Travel & Hospitality Industry Page · header with glass form
const { useState: useTvTop } = React;

function TvHeader({ onCTA }) {
  return (
    <section style={{ position: 'relative', overflow: 'hidden', background: 'var(--navy-900)' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${window.__resources.headerBg})`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.46, pointerEvents: 'none' }} role="img" aria-label="Sunset over a tropical beach representing travel and hospitality software" />
      ...
      <div style={{ position: 'relative', maxWidth: 1240, margin: '0 auto', padding: 'clamp(120px,12vw,150px) 32px clamp(72px,8vw,104px)', display: 'grid', gridTemplateColumns: '1.08fr 0.92fr', gap: 'clamp(40px,5vw,72px)', alignItems: 'center' }} className="tv-header-grid">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 26, fontFamily: 'Poppins, sans-serif', fontSize: 13.5, color: 'rgba(255,255,255,.6)' }}>
            <a href="Xorora Homepage.html" style={{ color: 'rgba(255,255,255,.6)', textDecoration: 'none' }}>Home</a>
            <Chevron size={13} color="rgba(255,255,255,.4)" />
            <span>Industries</span>
            <Chevron size={13} color="rgba(255,255,255,.4)" />
            <span style={{ color: '#fff', fontWeight: 600 }}>Travel & Hospitality</span>
          </div>

          <h1 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(34px,4.6vw,58px)', lineHeight: 1.04, letterSpacing: '-0.025em', color: '#fff', margin: '0 0 26px', textWrap: 'balance' }}>
            Travel and hospitality software development services<span style={{ color: 'var(--tangerine-400)' }}>.</span>
          </h1>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 22, flexWrap: 'wrap' }}>
            {[['shield-check', 'PCI DSS ready'], ['lock', 'GDPR compliant'], ['award', 'SOC 2 & ISO 27001']].map(([ic, l]) => (
...
function TvGlassForm() {
  const [sent, setSent] = useTvTop(false);
  const [nda, setNda] = useTvTop(false);
  return (
    <div id="tv-form" style={{
        ...
        {sent ? (
          ...
          <React.Fragment>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
              <TvField label="Full Name" placeholder="Jordan Reyes" />
              <TvField label="Email" placeholder="you@company.com" />
            </div>
            <div style={{ marginTop: 18 }}><TvField label="About project" placeholder="Tell us what you want to build" textarea /></div>
...
function TvField({ label, placeholder, textarea }) {
  const [f, setF] = useTvTop(false);
  const common = {
