// PingPanda Case Study — Result/Conclusion, Contact CTA
const { useState: usePpBot } = React;
  ...
  return (
    <LightSection bg="var(--surface)">
      ...
      <div style={{ position: 'relative', marginBottom: 40 }}>
        <BrowserFrame src={window.__resources.mockEvents} alt="PingPanda event API and live listening state" url="app.pingpanda.io/api-key" />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 20, marginBottom: 36 }}>
        {[
          { t: 'No missed signups', b: 'Revenue moments — signups, upgrades, payments — are tracked and alerted the instant they happen.' },
          { t: 'No delayed incident response', b: 'Errors, failures, and spikes reach the team in real time, where they already work.' },
          { t: 'No setup overhead', b: 'A single API endpoint and event categories — no dashboards to configure, no infrastructure to manage.' },
        ].map(c => (
      ...
      <div style={{ position: 'relative', overflow: 'hidden', background: 'var(--navy-900)', borderRadius: 'var(--r-xl)', padding: 'clamp(36px,5vw,56px)', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', display: 'flex', justifyContent: 'center', gap: 'clamp(20px,4vw,52px)', flexWrap: 'wrap' }}>
          {['Send an event.', 'Get an alert.', 'Never miss a moment.'].map((t, i) => (
...
function PpContactCTA({ innerRef }) {
  const [sent, setSent] = usePpBot(false);
  const [budget, setBudget] = usePpBot(null);
  const [hear, setHear] = usePpBot(null);
            ...
            {sent ? (
              ...
              <React.Fragment>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <PpField label="Your name" placeholder="Jordan Reyes" />
                  <PpField label="Work email" placeholder="you@company.com" />
                </div>
                <div style={{ marginTop: 14 }}><PpField label="How can we help?" placeholder="A sentence about your ambition…" textarea /></div>
                <PpChipRow label="Project budget" options={budgets} value={budget} onPick={setBudget} />
                <PpChipRow label="How did you hear about us?" options={hears} value={hear} onPick={setHear} />
...
function PpField({ label, placeholder, textarea }) {
  const [f, setF] = usePpBot(false);
...
function PpChipRow({ label, options, value, onPick }) {
  return (
...
Object.assign(window, { PpResult, PpContactCTA });
