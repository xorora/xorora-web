      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: 1240, margin: '0 auto', padding: 'clamp(96px,11vw,140px) 32px clamp(64px,7vw,96px)', display: 'grid', gridTemplateColumns: '1.08fr 0.92fr', gap: 'clamp(40px,5vw,72px)', alignItems: 'center' }} className="con-header-grid">
        <div>
          ...
          <h1 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(34px,4.6vw,60px)', lineHeight: 1.02, letterSpacing: '-0.03em', color: '#fff', margin: '0 0 24px', textWrap: 'balance' }}>
            IT consulting services that turn ambition into a <span style={{ background: 'linear-gradient(110deg,#C3C6EC,#8E94E0 46%,#FF8A3D)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>clear roadmap<span style={{ color: 'var(--tangerine-400)', WebkitTextFillColor: 'var(--tangerine-400)' }}>.</span></span>
          </h1>
...
function ConGlassForm() {
  ...
  return (
    <div id="con-hero-form" style={{ position: 'relative', borderRadius: 'var(--r-xl)', padding: 'clamp(28px,3vw,40px)', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.18)', backdropFilter: 'blur(18px)', WebkitBackdropFilter: 'blur(18px)', boxShadow: '0 30px 80px -30px rgba(0,0,0,0.7)' }} className="con-glass">
...
function ConApproach() {
  const steps = [
    { n: '01', icon: 'phone-call', name: 'Align', body: 'A discovery call to understand your business goals, constraints, and what success looks like.' },
    { n: '02', icon: 'search', name: 'Assess', body: 'We dig into your product, market, data, and systems to find the real problem worth solving.' },
    { n: '03', icon: 'map', name: 'Map', body: 'We translate findings into a costed, prioritized roadmap with architecture and a clear scope.' },
    { n: '04', icon: 'rocket', name: 'Activate', body: 'You leave ready to build, with us or your team. And we can deliver the roadmap end to end.' },
  ];
  return (
    <LightSection bg="var(--surface)">
      <SectionHead label="How we work" title="Our approach." sub="A proven engagement that gets you from uncertainty to a clear, costed plan." align="center" style={{ marginBottom: 52 }} />
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }} className="con-grid4">
        {steps.map(s => (
