...
/* TECH STACK */
function EngTechStack() {
  const tech = [
    ['atom', 'React'], ['atom', 'Next.js'], ['braces', 'TypeScript'], ['wind', 'Tailwind CSS'],
    ['triangle', 'Vue'], ['flame', 'Angular'], ['server', 'Node.js'], ['code', 'Python'],
    ['database', 'PostgreSQL'], ['leaf', 'MongoDB'], ['zap', 'Redis'], ['box', 'Docker'],
    ['ship', 'Kubernetes'], ['cloud', 'AWS'], ['cloud', 'GCP'], ['triangle', 'Vercel'],
    ['sparkles', 'OpenAI'], ['brain-circuit', 'LangChain'], ['credit-card', 'Stripe'], ['layout-template', 'WordPress'],
  ];
  ...
  return (
    <section style={{ background: 'var(--surface)', padding: 'clamp(72px,9vw,118px) 0', borderTop: '1px solid var(--border)', overflow: 'hidden' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto 48px', padding: '0 32px' }}>
        <SectionHead label="Engineering" title="Product engineering technology stacks." sub="We leverage modern frameworks, cloud platforms, and data technologies to build scalable engineering solutions." />
      </div>
      <div className="eng-tmq-wrap"><div className="eng-tmq eng-tmq-a">{rowA.map(([i, n], k) => <EngTechCard key={k} i={i} n={n} />)}</div></div>
      <div className="eng-tmq-wrap" style={{ marginTop: 16 }}><div className="eng-tmq eng-tmq-b">{rowB.map(([i, n], k) => <EngTechCard key={k} i={i} n={n} />)}</div></div>
    </section>
  );
}

function EngTechCard({ i, n }) {
  return (
    <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 12, background: 'var(--slate-50)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '15px 24px', boxShadow: 'var(--shadow-xs)' }}>
