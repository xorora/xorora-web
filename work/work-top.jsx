  ...
  return (
    <header style={{ position: 'sticky', top: 0, zIndex: 50, background: scrolled ? 'rgba(2,6,15,0.82)' : 'transparent', backdropFilter: scrolled ? 'blur(14px)' : 'none', borderBottom: `1px solid ${scrolled ? 'rgba(255,255,255,.08)' : 'transparent'}`, transition: 'all .25s ease' }}>
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: '16px 32px', display: 'flex', alignItems: 'center', gap: 38 }}>
        <a href="Homepage V2.html" style={{ cursor: 'pointer', display: 'flex' }}><Wordmark height={24} dark /></a>
...
/* ---------------- HEADER ---------------- */
function WorkHeader({ onStart, onWork }) {
  return (
    <div style={{ position: 'relative', overflow: 'hidden', background: '#02060F', marginTop: -72, paddingTop: 72 }}>
      <TechField />
      {/* drifting glow beams — distinct from the About aurora */}
      <div className="ow-beams" style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
        <span className="ow-beam ow-beam-1" />
        <span className="ow-beam ow-beam-2" />
        <span className="ow-beam ow-beam-3" />
      </div>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(120% 80% at 50% -10%, rgba(91,141,239,0.26) 0%, rgba(39,85,195,0.10) 32%, rgba(1,12,40,0) 62%)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(55% 50% at 50% 44%, rgba(2,6,15,0.5) 0%, rgba(2,6,15,0) 72%)', pointerEvents: 'none' }} />
      <XWatermark size={720} color="rgba(120,150,240,0.05)" style={{ top: -180, right: -160 }} />
      <div style={{ position: 'relative', maxWidth: 1100, margin: '0 auto', padding: 'clamp(124px,13vw,168px) 32px clamp(80px,9vw,116px)', textAlign: 'center' }}>
        ...
        <h1 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(44px,7vw,92px)', lineHeight: 0.98, letterSpacing: '-0.03em', color: '#fff', margin: '0 0 24px', textWrap: 'balance' }}>
          Work that <span style={{ background: 'linear-gradient(100deg, #C3C6EC, #8E94E0 45%, #FF8A3D)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>ships and scales<span style={{ color: 'var(--tangerine-400)', WebkitTextFillColor: 'var(--tangerine-400)' }}>.</span></span>
        </h1>
        <p style={{ fontFamily: 'Poppins, sans-serif', fontSize: 'clamp(17px,2vw,20px)', lineHeight: 1.6, color: 'rgba(255,255,255,.72)', maxWidth: 600, margin: '0 auto 38px' }}>
        ...
        <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
          <Button variant="light" size="lg" onClick={onStart}>Start a project <i data-lucide="arrow-up-right" style={{ width: 16 }}></i></Button>
          <Button variant="onDark" size="lg" onClick={onWork}>Explore our services</Button>
        </div>
      </div>
    </div>
  );
