// Xorora — EdTech Industry Page · header with glass form
const { useState: useEdTop } = React;

function EdHeader({ onCTA }) {
  return (
    <section style={{ position: 'relative', overflow: 'hidden', background: 'var(--navy-900)' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: "url('assets/edtech/header-bg.jpg')", backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.5, pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(95deg, rgba(1,12,40,0.97) 0%, rgba(1,12,40,0.9) 42%, rgba(3,12,38,0.64) 72%, rgba(3,12,38,0.5) 100%)', pointerEvents: 'none' }} />
      ...
      <XWatermark size={620} color="rgba(120,150,240,0.05)" style={{ top: -150, right: -150 }} />

      <div style={{ position: 'relative', maxWidth: 1240, margin: '0 auto', padding: 'clamp(120px,12vw,150px) 32px clamp(72px,8vw,104px)', display: 'grid', gridTemplateColumns: '1.08fr 0.92fr', gap: 'clamp(40px,5vw,72px)', alignItems: 'center' }} className="ed-header-grid">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 26, fontFamily: 'Poppins, sans-serif', fontSize: 13.5, color: 'rgba(255,255,255,.6)' }}>
            ...
            <Chevron size={13} color="rgba(255,255,255,.4)" />
            <span style={{ color: '#fff', fontWeight: 600 }}>EdTech</span>
          </div>
          ...
          <h1 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(36px,4.8vw,60px)', lineHeight: 1.02, letterSpacing: '-0.025em', color: '#fff', margin: '0 0 26px', textWrap: 'balance' }}>
            EdTech software development<span style={{ color: 'var(--tangerine-400)' }}>.</span>
          </h1>

          <p style={{ fontFamily: 'Poppins, sans-serif', fontSize: 18, lineHeight: 1.62, color: 'rgba(255,255,255,.74)', maxWidth: 540, margin: '0 0 36px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 22, flexWrap: 'wrap' }}>
            {[['shield-check', 'FERPA ready'], ['lock', 'GDPR compliant'], ['award', 'SOC 2 & ISO 27001']].map(([ic, l]) => (
...
function EdGlassForm() {
  const [sent, setSent] = useEdTop(false);
  const [nda, setNda] = useEdTop(false);
  return (
    <div id="ed-form" style={{
        ...
        {sent ? (
          ...
          <React.Fragment>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
              <EdField label="Full Name" placeholder="Jordan Reyes" />
              <EdField label="Email" placeholder="you@company.com" />
            </div>
            <div style={{ marginTop: 18 }}><EdField label="About project" placeholder="Tell us what you want to build" textarea /></div>
...
function EdField({ label, placeholder, textarea }) {
  const [f, setF] = useEdTop(false);
  const common = {
