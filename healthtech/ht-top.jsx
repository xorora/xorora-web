// Xorora — Health Tech Industry Page · header with glass form
const { useState: useHtTop } = React;

function HtHeader({ onCTA }) {
  return (
    <section style={{ position: 'relative', overflow: 'hidden', background: 'var(--navy-900)' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: "url('assets/healthtech/header-bg.jpg')", backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.5, pointerEvents: 'none' }} />
      ...
      <div style={{ position: 'relative', maxWidth: 1240, margin: '0 auto', padding: 'clamp(120px,12vw,150px) 32px clamp(72px,8vw,104px)', display: 'grid', gridTemplateColumns: '1.08fr 0.92fr', gap: 'clamp(40px,5vw,72px)', alignItems: 'center' }} className="ht-header-grid">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 26, fontFamily: 'Poppins, sans-serif', fontSize: 13.5, color: 'rgba(255,255,255,.6)' }}>
            ...
            <Chevron size={13} color="rgba(255,255,255,.4)" />
            <span style={{ color: '#fff', fontWeight: 600 }}>Health Tech</span>
          </div>
          ...
          <h1 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(36px,4.8vw,60px)', lineHeight: 1.02, letterSpacing: '-0.025em', color: '#fff', margin: '0 0 26px', textWrap: 'balance' }}>
            HealthTech software development<span style={{ color: 'var(--tangerine-400)' }}>.</span>
          </h1>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 22, flexWrap: 'wrap' }}>
            {[['shield-check', 'HIPAA ready'], ['lock', 'GDPR compliant'], ['award', 'SOC 2 & ISO 27001']].map(([ic, l]) => (
...
function HtGlassForm() {
  const [sent, setSent] = useHtTop(false);
  const [nda, setNda] = useHtTop(false);
  return (
    <div id="ht-form" style={{
        ...
        {sent ? (
          ...
          <React.Fragment>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
              <HtField label="Full Name" placeholder="Jordan Reyes" />
              <HtField label="Email" placeholder="you@company.com" />
            </div>
            <div style={{ marginTop: 18 }}><HtField label="About project" placeholder="Tell us what you want to build" textarea /></div>
...
function HtField({ label, placeholder, textarea }) {
  const [f, setF] = useHtTop(false);
  const common = {
