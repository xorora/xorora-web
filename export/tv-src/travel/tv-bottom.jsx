// Xorora — Travel & Hospitality Industry Page · Process (dark) + FAQ + CTA
const { useState: useTvBot } = React;

function TvProcess() {
  const steps = [
    { n: '01', name: 'Project discovery', body: 'First, we analyze your business goals, properties, and guest journey to create clearly articulated goals, optimize processes, and isolate the best opportunities.' },
    { n: '02', name: 'Team composition', body: 'We assemble multidisciplinary teams with travel, data, and compliance backgrounds, matched to your project scope, seasonality, and growth plans.' },
    { n: '03', name: 'Project delivery', body: 'From MVP through enterprise scale implementation, we build booking engines, PMS, guest apps, and scalable architectures for sustained success.' },
  ];
  return (
    <DarkSection bloom="50% 4%">
      <SectionHead label="How we build" title="How we build scalable travel solutions." onDark align="center" style={{ marginBottom: 56 }} />
      ...
      <div style={{ textAlign: 'center', marginTop: 8 }}>
        ...
        <Button variant="primary" size="lg" onClick={() => { const el = document.getElementById('tv-form'); if (el) window.scrollTo({ top: el.getBoundingClientRect().top + window.scrollY - 100, behavior: 'smooth' }); }}>Request a consultation <i data-lucide="arrow-up-right" style={{ width: 16 }}></i></Button>
      </div>
    </DarkSection>
  );
}
...
function TvFAQ() {
  const faqs = [
    { q: 'What types of travel and hospitality software can you build?', a: 'We build booking and reservation engines, property management systems, travel portals, guest and loyalty apps, channel managers, and analytics dashboards, all tailored to your operation.' },
    { q: 'Do you work as a travel app development company and a restaurant app development company?', a: 'Yes. As a travel app development company and restaurant app development company, we build mobile and web apps for hotels, agencies, tour operators, and restaurants, including a business travel management app for corporate travel programs.' },
    { q: 'Can you integrate with PMS, GDS, and OTA channels?', a: 'Yes. We integrate with property management systems, global distribution systems, OTA channels, payment providers, and review platforms through robust APIs so your data stays in sync.' },
    { q: 'What are the typical project timelines?', a: 'A focused MVP usually ships in a few months, while a full platform with integrations runs longer. After discovery we give a clear, milestone based timeline.' },
    { q: 'How do you handle peak season traffic and scale?', a: 'We architect for elastic scaling, caching, and load balancing so your booking flows stay fast and available during launches, sales, and seasonal peaks.' },
    { q: 'How do you use AI in travel platforms?', a: 'We apply machine learning for recommendations, dynamic pricing, and demand forecasting, and use modern AI for multilingual guest support where it adds measurable value.' },
    { q: 'Do you provide post launch maintenance and support?', a: 'Yes. We provide ongoing maintenance, monitoring, and DevOps so your platform stays secure, reliable, and ready to scale as your bookings grow.' },
  ];
  const [open, setOpen] = useTvBot(0);
  return (
    <LightSection bg="var(--surface)">
      <div style={{ display: 'grid', gridTemplateColumns: '0.7fr 1.3fr', gap: 'clamp(32px,5vw,64px)', alignItems: 'start' }} className="tv-2col">
          ...
          {faqs.map((f, i) => {
            ...
            return (
              <div key={i} style={{ borderTop: '1px solid var(--border)' }}>
                ...
                {isOpen && <p style={{ fontFamily: 'Poppins, sans-serif', fontSize: 15.5, lineHeight: 1.7, color: 'var(--fg2)', margin: '0 0 24px', maxWidth: 680, animation: 'tvFade .25s ease' }}>{f.a}</p>}
              </div>
            );
          })}
        </div>
  ...
  return (
    <section style={{ background: 'var(--surface)', padding: '0 32px clamp(64px,8vw,104px)' }}>
      <div style={{ position: 'relative', overflow: 'hidden', maxWidth: 1240, margin: '0 auto', background: 'var(--navy-900)', borderRadius: 'var(--r-xl)', padding: 'clamp(44px,6vw,80px)' }}>
        <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${window.__resources.headerBg})`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.2, pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 32, flexWrap: 'wrap' }}>
          <div style={{ maxWidth: 640 }}>
            ...
            <h2 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(30px,4vw,52px)', lineHeight: 1.04, letterSpacing: '-0.025em', color: '#fff', margin: '0 0 18px', textWrap: 'balance' }}>
              Scale your vision with next generation travel solutions.
            </h2>
...
Object.assign(window, { TvProcess, TvFAQ, TvCTA });
