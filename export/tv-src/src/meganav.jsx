...
function MegaNav({ onLetsTalk, current }) {
  ...
  function handleLetsTalk(e) {
    if (onLetsTalk) { e.preventDefault(); onLetsTalk(); }
  }
    ...
    <div onMouseLeave={leaveAll} style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, pointerEvents: 'none' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', padding: '18px 20px 0' }}>
        <div style={{
          ...
          <a href="Xorora Homepage.html" style={{ position: 'relative', display: 'flex', alignItems: 'center', height: 36, width: scrolled ? 40 : 150, transition: 'width .35s cubic-bezier(.4,0,.2,1)', flexShrink: 0 }}>
            <img src={window.__resources.navFull} alt="Xorora" style={{ position: 'absolute', left: 0, height: 28, opacity: scrolled ? 0 : 1, transition: 'opacity .3s ease' }} />
            <img src={window.__resources.navX} alt="Xorora" style={{ position: 'absolute', left: 0, height: 28, opacity: scrolled ? 1 : 0, transition: 'opacity .3s ease' }} />
          </a>
      ...
      {open && (
        <div onMouseEnter={() => clearTimeout(closeTimer.current)} className="xo-mn-panelwrap"
          ...
          <div style={{
            ...
            {open === 'Services' && <ServicesPanel D={D} onPick={() => setOpen(null)} onLetsTalk={onLetsTalk} />}
            {open === 'Industries' && <IndustriesPanel D={D} />}
            {open === 'Company' && <CompanyPanel D={D} onLetsTalk={onLetsTalk} />}
          </div>
        </div>
      )}

      {/* MOBILE MENU */}
      {mobile && <MobileMenu D={D} onClose={() => setMobile(false)} onLetsTalk={onLetsTalk} />}
    </div>
  );
}
...
function ServicesPanel({ D, onPick, onLetsTalk }) {
  return (
    <div className="xo-mn-services" style={{ display: 'grid', gridTemplateColumns: '256px 1fr', minHeight: 360 }}>
      {/* promo */}
      <div style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(160deg, var(--navy-800), var(--navy-950))', padding: '34px 30px', display: 'flex', flexDirection: 'column', borderRight: '1px solid rgba(255,255,255,.08)' }}>
        <div style={{ position: 'absolute', bottom: -120, left: -40, width: 320, height: 320, borderRadius: '50%', background: 'repeating-radial-gradient(circle at center, rgba(120,150,240,.12) 0px, rgba(120,150,240,.12) 1px, transparent 1px, transparent 22px)', opacity: .7 }} />
        ...
        {D.services.map(cat => (
          <div key={cat.name} style={{ padding: '8px 10px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {cat.items.map(([label]) => (
                <a key={label} href="#contact" onClick={e => { if (onLetsTalk) { e.preventDefault(); onLetsTalk(); } onPick(); }}
...
function IndustriesPanel({ D }) {
  return (
    <div className="xo-mn-industries" style={{ display: 'grid', gridTemplateColumns: '1fr 380px' }}>
      ...
      {/* featured */}
      <a href={D.featured.href} style={{ position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', textDecoration: 'none', background: 'linear-gradient(160deg, var(--navy-800), var(--navy-950))', borderLeft: '1px solid rgba(255,255,255,.08)', padding: '26px 26px' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(90% 70% at 90% 0%, rgba(91,141,239,0.22), transparent 60%)', pointerEvents: 'none' }} />
...
function CompanyPanel({ D, onLetsTalk }) {
  return (
    <div className="xo-mn-company" style={{ display: 'grid', gridTemplateColumns: '1fr 340px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px 10px', padding: '26px 26px' }}>
        {D.company.map(c => (
          <a key={c.name} href="#" onClick={e => e.preventDefault()}
      ...
      {/* book a meeting CTA */}
      <div style={{ position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: 'linear-gradient(160deg, var(--indigo-700), var(--indigo-900))', borderLeft: '1px solid rgba(255,255,255,.08)', padding: '30px 28px' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(80% 70% at 90% 0%, rgba(120,150,240,0.35), transparent 60%)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative' }}>
...
function MobileMenu({ D, onClose, onLetsTalk }) {
  ...
  return (
    <div className="xo-mn-mobile" style={{ pointerEvents: 'auto', margin: '10px 20px 0', maxHeight: 'calc(100vh - 120px)', overflowY: 'auto', background: 'rgba(8,12,30,0.98)', border: '1px solid rgba(255,255,255,.12)', borderRadius: 'var(--r-xl)', padding: '12px 20px 24px', boxShadow: '0 40px 90px -30px rgba(0,0,0,0.8)' }}>
      ...
      {sec === 'Company' && <div style={{ padding: '8px 0 14px' }}>{D.company.map(c => sub(c.name))}</div>}
