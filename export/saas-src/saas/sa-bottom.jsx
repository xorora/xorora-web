// Xorora — SaaS Industry Page · Process (dark) + FAQ + CTA
const { useState: useSaBot } = React;

function SaProcess() {
  const steps = [
    { n: '01', name: 'Project discovery', body: 'First, we analyze your product goals, customers, and workflows to create clearly articulated goals, optimize processes, and isolate the best opportunities to ship value fast.' },
    { n: '02', name: 'Team composition', body: 'We assemble multidisciplinary teams with SaaS, data, and compliance backgrounds, matched to your project scope, scale requirements, and growth plans.' },
    { n: '03', name: 'Project delivery', body: 'From MVP through enterprise scale implementation, we build multi tenant platforms, billing, dashboards, and scalable architectures for sustained success.' },
  ];
  return (
    <DarkSection bloom="50% 4%">
      <SectionHead label="How we build" title="How we build scalable SaaS products." onDark align="center" style={{ marginBottom: 56 }} />
      ...
      <div style={{ textAlign: 'center', marginTop: 8 }}>
        ...
        <Button variant="primary" size="lg" onClick={() => { const el = document.getElementById('sa-form'); if (el) window.scrollTo({ top: el.getBoundingClientRect().top + window.scrollY - 100, behavior: 'smooth' }); }}>Request a consultation <i data-lucide="arrow-up-right" style={{ width: 16 }}></i></Button>
      </div>
    </DarkSection>
  );
}
...
function SaFAQ() {
  const faqs = [
    { q: 'What types of SaaS software can you build?', a: 'We build multi tenant platforms, admin and customer dashboards, billing and subscription systems, internal tools, B2B portals, and analytics products, all tailored to your business model.' },
    { q: 'How do you handle multi tenancy and data isolation?', a: 'We architect tenancy with strict data isolation, per tenant configuration, and scalable provisioning so one customer never affects another and onboarding stays fast.' },
    { q: 'Can you integrate billing and third party services?', a: 'Yes. We integrate Stripe and other billing providers, auth systems, analytics, and partner APIs through robust integrations so your stack stays in sync.' },
    { q: 'What are the typical project timelines for SaaS development?', a: 'A focused MVP usually ships in a few months, while a full platform with integrations runs longer. After discovery we give a clear, milestone based timeline.' },
    { q: 'How do you use AI in SaaS products?', a: 'We apply machine learning for search, recommendations, and analytics, and use modern AI for copilots, automation, and content where they add measurable value.' },
    { q: 'Do you provide post launch maintenance and support?', a: 'Yes. We provide ongoing maintenance, monitoring, and DevOps so your platform stays secure, reliable, and ready to scale as your customer base grows.' },
  ];
  const [open, setOpen] = useSaBot(0);
  return (
    <LightSection bg="var(--surface)">
      <div style={{ display: 'grid', gridTemplateColumns: '0.7fr 1.3fr', gap: 'clamp(32px,5vw,64px)', alignItems: 'start' }} className="sa-2col">
          ...
          {faqs.map((f, i) => {
            ...
            return (
              <div key={i} style={{ borderTop: '1px solid var(--border)' }}>
                ...
                {isOpen && <p style={{ fontFamily: 'Poppins, sans-serif', fontSize: 15.5, lineHeight: 1.7, color: 'var(--fg2)', margin: '0 0 24px', maxWidth: 680, animation: 'saFade .25s ease' }}>{f.a}</p>}
              </div>
            );
          })}
        </div>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 32, flexWrap: 'wrap' }}>
          <div style={{ maxWidth: 640 }}>
            ...
            <h2 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(30px,4vw,52px)', lineHeight: 1.04, letterSpacing: '-0.025em', color: '#fff', margin: '0 0 18px', textWrap: 'balance' }}>
              Scale your vision with next generation SaaS solutions.
            </h2>
...
Object.assign(window, { SaProcess, SaFAQ, SaCTA });
